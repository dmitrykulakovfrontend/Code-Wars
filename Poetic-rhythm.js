...
const symbols = [...syllables.short, ...syllables.long]
// return the type of meter
const identifyMeter = str => {
  if ([...str].some(char => !symbols.includes(char))) return "What is this?";
  if (str.length !== 2) return "Not a dissyllable.";
  const firstIsLong = syllables.long.includes(str[0]);
  const secondIsLong = syllables.long.includes(str[1]);
  if (firstIsLong && !secondIsLong) return "trochee";
  if (!firstIsLong && secondIsLong) return "iamb";
  if (firstIsLong && secondIsLong) return "spondee";
  if (!firstIsLong && !secondIsLong) return "pyrrhic";
}
