/*
  ...
  It's used by kids in several screenplays created by Astrid Lindgren.

  The idea is that every consonant in a sentence is duplicated, with an o inserted in between (b becomes bob).
  Vowels are left untouched (a remains an a).
  ...
  The Assignment
  Your task is to implement a function function robberEncode(sentence) which takes in a string sentence and returns the result, converted into
  robber language, as a string.
...
function robberEncode(sentence) {
  ...
  for (let char of sentence) {
    if (consonants.includes(char)) {
      result += `${char}${char.toUpperCase() === char ? 'O' : 'o'}${char}`
    } else result += char;
  }
  return result;
}
