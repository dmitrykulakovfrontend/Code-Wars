/*
  ...
  In many large arenas the crowd is seated in a contiguous circuit all the way around the sport field, and so the wave is able to travel
  continuously around the arena; in discontiguous seating arrangements, the wave can instead reflect back and forth through the crowd.
...
function wave(str){
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    for (let j = 0; j < str.length; j++) {
      if (j === i) {
        mexicanStr += str[j].toUpperCase();
      } else {
        mexicanStr += str[j]
      }
    }
    results.push(mexicanStr);
  }
  return results;
}
