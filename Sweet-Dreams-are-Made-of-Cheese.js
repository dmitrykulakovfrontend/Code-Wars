...
function payCheese(arr) {
  //Calculate the total number of wheels produced for the week
  const totalWheels = arr.reduce((acc, val) => acc + val);
  
  //Calculate the total hours worked for the week
  const totalHours = Math.ceil(totalWheels / 10 * (6 / 60));
  
  //Calculate the total staff wages for the week
  const totalWages = totalHours * 8.75 * 4;
  
  //Return the total staff wages as a string with the £ symbol
  return "£" + totalWages;
}
