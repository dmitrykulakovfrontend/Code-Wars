/*
  Bubblesort is an inefficient sorting algorithm that is simple to understand and therefore often taught in introductory computer
  science courses as an example how not to sort a list.
  ...
  language's naming conventions) should return a new array equivalent to performing exactly 1 complete pass on the original array.
...
function bubblesortOnce(a) {
  const copy = [...a];
  for (let i = copy.length; i > copy.length - 1; i--) {
    for (let j = 0; j < i; j++) {
      if (copy[j] > copy[j + 1]) {
